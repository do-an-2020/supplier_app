...
class Index extends Component {
  ...
  static onShow = (children, config = {}) => {
    if (!children || children.lengh > 1) return
  ...
  constructor(props) {
    ...
    this.gestureEvent = Animated.event(
      [{ nativeEvent: { translationX: this.gestureX, translationY: this.gestureY } }],
      {
    ...
    this.test = Animated.add(this.gestureY, Animated.multiply(-1, this.beginScrollY))
  ...
  onHandlerStateChange = e => {
    ...
    if (state === 5) {
      ...
      if (
        (this.styleTranslateY.__getValue() < -Math.max(contentHeight - layoutHeight + 200, 200) &&
          !this.type) ||
  ...
  render() {
    ...
    if (contentHeight > layoutHeight) {
      translateY = this.styleTranslateY.interpolate({
        ...
        outputRange: [-layoutHeight, -layoutHeight, 0, 0, layoutHeight, layoutHeight],
        extrapolate: 'clamp',
      })
    } else {
      translateY = this.styleTranslateY.interpolate({
        inputRange: [-layoutHeight - 1, -layoutHeight, 0, 0, layoutHeight, layoutHeight + 1],
        outputRange: [-layoutHeight, -layoutHeight, 0, 0, layoutHeight, layoutHeight],
        extrapolate: 'clamp',
      })
    }
          ...
          <PanGestureHandler
            ...
            <NativeViewGestureHandler
              ...
              {this.config.isUseScrollView ? (
                ...
                <Animated.View style={{ flex: 1 }}>{children || childProp}</Animated.View>
              )}
            </NativeViewGestureHandler>
          </PanGestureHandler>
        </Animated.View>
